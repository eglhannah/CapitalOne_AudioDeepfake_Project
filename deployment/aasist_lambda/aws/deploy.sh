...
if [[ -z "${DEMO_PASSPHRASE:-}" ]]; then
  echo "Set DEMO_PASSPHRASE before deploying." >&2
  exit 1
fi
...
echo "Deploying ${PROJECT_NAME} to account ${ACCOUNT_ID} in ${AWS_REGION}"
echo "S3 website origin: ${WEBSITE_ENDPOINT}"
...
# IAM propagation is eventually consistent; give Lambda a moment before create/update.
sleep 10

ENV_FILE="$(mktemp)"
DEMO_PASSPHRASE="$DEMO_PASSPHRASE" WEBSITE_ENDPOINT="$WEBSITE_ENDPOINT" "$PYTHON_BIN" - <<'PY' > "$ENV_FILE"
import json
import os
...
print(json.dumps({
    "Variables": {
        ...
        "DEMO_ALLOWED_ORIGIN": os.environ["WEBSITE_ENDPOINT"],
    }
}))
PY
