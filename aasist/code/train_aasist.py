#!/usr/bin/env python
"""Train AASIST on ASVspoof 2019 LA, reusing Chase's pipeline."""
...
def build_loaders(args):
    ...
    tr_ds = ASVspoofLADataset(tr_items, sample_rate=16000, duration_sec=64600/16000, training=True)
    dv_ds = ASVspoofLADataset(dv_items, sample_rate=16000, duration_sec=64600/16000, training=False)
    tr_ld = DataLoader(tr_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                       pin_memory=True, drop_last=True)
    dv_ld = DataLoader(dv_ds, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
                       pin_memory=True)
    return tr_ld, dv_ld
...
def evaluate(model, loader, criterion, device):
    ...
    for batch in tqdm(loader, desc="eval", leave=False):
        ...
        bs = x.size(0); tot += loss.item() * bs; n += bs
        labels.extend(y.cpu().numpy().tolist()); scores.extend(probs.cpu().numpy().tolist())
...
def main():
    ...
    run_name = args.run_name or f"aasist_{ts}"
    run_dir = OUTPUT_DIR / run_name; run_dir.mkdir(parents=True, exist_ok=True)
    print(f"run dir: {run_dir}")
    (run_dir / "config.json").write_text(json.dumps({**vars(args), "model": AASIST_CFG}, indent=2))
    ...
    for ep in range(1, args.epochs + 1):
        ...
        sched.step()
        history.append({"epoch": ep, "train_loss": tr_loss, **{f"dev_{k}": v for k, v in dv.items()}})
        (run_dir / "history.json").write_text(json.dumps(history, indent=2))
        print(f"ep{ep:03d}/{args.epochs} | tr_loss={tr_loss:.4f} | dv_loss={dv['loss']:.4f} | "
              f"dv_eer={dv['eer']:.4f} | dv_auc={dv['roc_auc']:.4f} | dv_acc={dv['accuracy']:.4f}")
        ...
        if math.isfinite(dv["eer"]) and dv["eer"] < best_eer:
            best_eer = dv["eer"]; torch.save(ckpt, run_dir / "best.pt")
